...
// Actions:
//   { action: "request", email }                 → send 6-digit OTP to admin email
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Helper: confirm this email is an admin user
    ...
    if (action === "request") {
      ...
      // Always respond success to avoid email enumeration; only send if admin
      if (user) {
        const code = genCode();
        ...
        await supabase.from("admin_password_otps").update({ used: true }).eq("email", email).eq("used", false);
        await supabase.from("admin_password_otps").insert({ email, code_hash, expires_at });
        ...
        const html = `
          <div style="font-family:Arial,sans-serif;max-width:560px;margin:0 auto;padding:24px;background:#f9fafb">
            <div style="background:#fff;border-radius:12px;padding:32px;box-shadow:0 2px 8px rgba(0,0,0,.06)">
              <h2 style="margin:0 0 8px;color:#111">Admin Password Reset</h2>
              <p style="color:#555;margin:0 0 24px">আপনার অ্যাডমিন পাসওয়ার্ড রিসেট করার জন্য নিচের OTP কোডটি ব্যবহার করুন।</p>
        ...
        await supabase.functions.invoke("send-email", {
          body: { to: email, subject: "Admin Password Reset OTP", html, text: `Your OTP: ${code} (valid 10 minutes)` },
        });
      }
      return json({ ok: true, message: "If this email is an admin, an OTP has been sent." });
    }
    ...
    if (action === "verify") {
      ...
      if (rec.code_hash !== code_hash) {
        await supabase.from("admin_password_otps").update({ attempts: (rec.attempts ?? 0) + 1 }).eq("id", rec.id);
        return json({ ok: false, error: "Incorrect OTP" }, 400);
      }

      // Generate a one-time reset token (valid 10 minutes from now via expires_at)
    ...
    if (action === "reset") {
      ...
      if (new_password.length < 8) return json({ ok: false, error: "Password must be at least 8 characters" }, 400);
      ...
      const user = await getAdminUser();
      if (!user) return json({ ok: false, error: "Admin not found" }, 404);

      const { error: upErr } = await supabase.auth.admin.updateUserById(user.id, { password: new_password });
      if (upErr) return json({ ok: false, error: upErr.message }, 500);

      await supabase.from("admin_password_otps").update({ used: true }).eq("id", rec.id);
